...
| ✅ Candidate evidence failed after green staging E2E because `referencing` was absent on the runner | Candidate-evidence now installs the pinned release-contract requirements before its producer-set validation; its setup-python pin was also corrected after the first merged-main rerun exposed an unresolvable copied SHA |
