...
# Cost when running: ~$41.66/month (Spot) or ~$17-40/month depending on usage.
###############################################################################
...
echo "[1/7] Checking if ALB already exists..."
...
if [ "$NEEDS_ALB" = true ]; then
  echo ""
  echo "[2/7] Creating ALB ($ALB_NAME)..."
...
if [ "$ALREADY_HAS_TG" = false ] || [ "$NEEDS_ALB" = true ]; then
  echo ""
  echo "[3/7] Creating target group ($TG_NAME)..."
  ...
  echo ""
  echo "[4/7] Creating listener (port 80 → $TG_NAME)..."
...
echo ""
echo "[5/7] Wiring API Gateway ECS service to target group..."
...
echo ""
echo "[6/7] Scaling all ECS services to desired-count=1..."
...
echo ""
echo "[7/7] Waiting for all services to reach steady state (up to 5 minutes)..."
...
echo ""
echo "Estimated running cost (Spot + ALB 24/7): ~\$41.66/month"
