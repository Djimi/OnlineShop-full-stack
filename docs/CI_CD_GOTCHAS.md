...
| `Unknown parameter in input: "Names", must be one of: LoadBalancerNames, Marker, PageSize` | ELBv2 `describe-load-balancers` takes `LoadBalancerNames`, not `Names` (that's `describe-target-groups`); a fake client mirroring the wrong parameter hides the bug until a live run | Call `describe_load_balancers(LoadBalancerNames=[name])`; keep fake clients parameter-named identically to the real API and add a lifecycle test without `e2eBaseUrl` so the ALB DNS path is exercised |
